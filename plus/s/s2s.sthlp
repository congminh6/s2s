...
to{cmd:(}{it:numlist}{cmd:)} pline{cmd:(}{it:varname}{cmd:)} cluster{cmd:(}{it:varname}{cmd:)} [method{cmd:(}{it:string}{cmd:)} {it:options}]
...
{phang}
{opt from(#)} specifies the survey year (or round) that has consumption data and that provides the underlying regression for imputation. This is the base survey that we impute from. The number for this base survey takes one of the values specified in the variable used in the {opt by(varname)} option. For example, if the year variable has two values 2008 and 2010, either of which can be specified as a number to be used.

{phang}
{opt to(#)} specifies the survey year (or round) that has missing consumption data and needs to be imputed into. This is the target survey that we impute into. The number for this survey year takes one of the values specified in the variable used in the {opt by(varname)} option. For example, if the year variable has two values 2008 and 2010, either of which can be specified as a number to be used.

{phang}
{opt method(string)} specifies the imputation method. Four methods are allowed.

{pmore} 
{cmd:normal}: using the linear regression (OLS) model with the distributions of the error terms assumed to be normal. This is the default option.

{pmore} 
{cmd:empirical}: using the linear regression (OLS) model with the empirical distributions of the error terms.

{pmore}
{cmd:probit}: using the probit regression model. This option only applies to headcount poverty. If this option is specified, the estimated standard errors are analytical.

{pmore}
{cmd:logit}: using the logit regression model. This option only applies to headcount poverty. If this option is specified, the estimated standard errors are analytical.
...
{phang}
{opt pline2(varname)} specifies the variable that indicates the extreme poverty line. The extreme poverty line should be lower than the (regular) poverty line that is specified in the option {opt pline()}. This variable has a numeric format.
...
{phang}
{opt lny} specifies that the left-hand side variable is converted to logarithmic terms. Note that the poverty line variables always remain in level terms.
...
{pstd} For illustration, assume that consumption data are available in the 2019/20 survey round but are either missing or not comparably constructed in the 2020/2021 survey round. We impute from the 2019/20 survey into the 2020/21 survey. The outcomes of interest include {p_end}
...
{pstd} We use the normal linear regression model, with the distribution of the error terms assumed to be normal. The given poverty line is 475630.7 TZS, and the given extreme poverty line is 377785.1 TZS (both poverty lines are annual and per adult equivalent). The vulnerability line is specified at 125% of the poverty line in 2019.{p_end}
...
{pstd}* Model 1 {p_end}       
{phang2}{cmd:. s2s dep hhsize age female pri lws ups age0to14sh age15to24sh age60tosh wageemp selfemp area_1 area_2 area_4, by(year) from(2019) to(2021) pline(povline) method(normal) cluster(psu) wt(hhszwt) strata(strata) pline2(epovline) vline(vline) lny rep(1000) brep(1000)}{p_end}
...
{pstd} We can generate a dataset with imputed consumption for each household. The example below shows a dataset ("predicted_data.dta") that has 100 imputed consumption variables for each household. The number of imputed consumption variables is specified by the number in the {opt rep()} option. The imputed data is saved in the folder C:\Temp\.{p_end}

{phang2}{cmd:. use Tanzania_dataset.dta, clear}{p_end}
{phang2}{cmd:. clonevar dep = lnpcex}{p_end}
{phang2}{cmd:. replace dep = . if year == 2021}{p_end}
{phang2}{cmd:. s2s lnpcex hhsize age female pri lws ups age0to14sh age15to24sh age60tosh wageemp selfemp area_1 area_2 area_4 lnpcewg, by(year) from(2019) to(2021) pline(povline) method(normal) cluster(psu) wt(hhszwt) strata(strata) lny rep(100) brep(100) saving("C:\Temp\predicted_data.dta") replace}{p_end}

{dlgtab:Example 3}

{pstd} We can give more weight to poorer people by using a higher value for alpha in the FGT index, including the variant of poverty gap that focuses on the poor population.{p_end}

{phang2}{cmd:. use Tanzania_dataset.dta, clear}{p_end}
{phang2}{cmd:. clonevar dep = lnpcex}{p_end}
{phang2}{cmd:. replace dep = . if year == 2021}{p_end}
{phang2}{cmd:. s2s dep hhsize age female pri lws ups age0to14sh age15to24sh age60tosh wageemp selfemp area_1 area_2 area_4 lnpcewg, by(year) from(2019) to(2021) pline(povline) method(normal) cluster(psu) wt(hhszwt) strata(strata) lny rep(10) brep(100) a(5)}{p_end}
...
{phang2}{cmd:. clonevar dep = lnpcex}{p_end}
{phang2}{cmd:. replace dep = . if year == 2021}{p_end}
{phang2}{cmd:. s2s dep hhsize age female pri lws ups age0to14sh age15to24sh age60tosh wageemp selfemp area_1 area_2 area_4 lnpcewg, by(year) from(2019) to(2021) pline(povline) method(probit) cluster(psu) wt(hhszwt) strata(strata) lny rep(1000) brep(1000)}{p_end}

{dlgtab:Example 5}

{pstd} We can use the empirical distribution of the error terms instead of the default normal linear regression. {p_end}

{phang2}{cmd:. use Tanzania_dataset.dta, clear}{p_end}
{phang2}{cmd:. clonevar dep = lnpcex}{p_end}
{phang2}{cmd:. replace dep = . if year == 2021}{p_end}
{phang2}{cmd:. s2s dep hhsize age female pri lws ups age0to14sh age15to24sh age60tosh wageemp selfemp area_1 area_2 area_4 lnpcewg, by(year) from(2019) to(2021) pline(povline) method(empirical) cluster(psu) wt(hhszwt) strata(strata) lny rep(1000) brep(1000) a(1)}{p_end}
...
{p 4 4 2} Dang, H.-A. H., Lanjouw, P. F., & Serajuddin, U. (2017). Updating poverty estimates in the absence of regular and comparable consumption data: methods and illustration with reference to a middle-income country. {it:Oxford Economic Papers,} 69(4), 939-962.{p_end}
...
{p 4 4 2} Dang, H. A. H., Nguyen, M. C., and Abanokova, K. (2025). s2s: Stata module to impute poverty in the absence of consumption (income) data. World Bank, Development Data Group and Global Poverty Department. Available from (insert Repec link) {p_end}
